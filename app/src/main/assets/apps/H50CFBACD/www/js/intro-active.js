...
document.addEventListener('plusready',function(){
  ...
  $('.topimg').on('click',function(){
		// 弹出系统选择按钮框
		plus.nativeUI.actionSheet( {cancel:"取消",buttons:[{title:"拍照添加"},{title:"相册添加"},{title:"从本地上传视频"}]}, function(e){
			if(e.index==1){
			    ss=1;
				getImage();
			}else if(e.index==2){
...
//			console.log(JSON.stringify(err));
	...
	function appendByGalleryMul() {
		plus.gallery.pick(function (e){
			var html = '';console.log(JSON.stringify(e));
...
//			console.log(JSON.stringify(e));
		},{multiple:true,system:false,maximum:9,onmaxed:function(){
			  toast('最多只能选择9张图片')
		}});
	...
	function simg(){
       ...
       if(v){ 
			if(confirm('是否确认删除！')){
			    ...
			    setTimeout(function(){v=true;},1000);
			    $(this).detach()
...
//					console.log(pic);
					...
					if(pic != '-1'){
						console.log(apiRoot +'/Home/Active/introactive/user'+userid+"/body/"+content+"/imagess/"+pic)
						//保存到数据库
						$.ajax({
							type :"get",
							url:apiRoot +"/Home/Active/introactive",
